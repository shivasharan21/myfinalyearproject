...
function DoctorDashboard() {
  ...
  const fetchDashboardData = useCallback(async (showRefreshing = false) => {
    try {
      ...
      // Build the scoped patient list from all three sources so Prescriptions
      // and HealthRecords always show the same restricted set of patients.
  ...
  useEffect(() => {
    ...
    const handleNewAppointment = (data) => {
      ...
      addNotification({
        ...
        message: `${data.appointment?.patientName || "A patient"} requested an appointment`,
        autoClose: true,
      });
    };
    ...
    // ─── Prescription and Health Record WebSocket listeners ───────────────────
    const handlePrescriptionCreated = (data) => {
      console.log("💊 Prescription created via WebSocket:", data);
      addNotification({
        type: "success",
        title: "Prescription Created",
        message: `Prescription for ${data.prescription?.patientId?.name || "patient"} has been created`,
        autoClose: true,
      });
    };

    const handlePrescriptionUpdated = (data) => {
      console.log("💊 Prescription updated via WebSocket:", data);
      addNotification({
        type: "success",
    ...
    const handleHealthRecordCreated = (data) => {
      console.log("📋 Health record created via WebSocket:", data);
      addNotification({
        type: "success",
        title: "Health Record Created",
        message: `Health record "${data.record?.title}" has been created for the patient`,
        autoClose: true,
      });
    };

    const handleHealthRecordUpdated = (data) => {
      console.log("📋 Health record updated via WebSocket:", data);
      addNotification({
        type: "success",
  ...
  const answerCall = () => {
    ...
    const appointment = appointments.find(
      (a) => a._id === incomingCall.appointmentId,
    );
    if (appointment) {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "patients":
        return <PatientList appointments={appointments} doctorPatients={doctorPatients} />;
      case "schedule":
        return <ScheduleView appointments={appointments} />;
      case "reports":
        return <Reports stats={stats} appointments={appointments} />;
      case "prescriptions":
        return <Prescriptions doctorPatients={doctorPatients} />;
      case "health-records":
        return <HealthRecords doctorPatients={doctorPatients} />;
      default:
        return null;
    }
  };
          ...
          <div className="p-4 flex-1">
            ...
            <nav className="space-y-1">
              ...
              <NavButton
                ...
                active={activeTab === "health-records"}
...
function NavButton({ icon, label, active, onClick, count }) {
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-4 py-3 rounded-xl flex items-center justify-between transition-all duration-200 text-sm font-medium ${active ? "bg-gradient-to-r from-cyan-600 to-blue-600 text-white shadow-md" : "text-gray-700 hover:bg-gray-100"}`}
    >
      ...
      {count > 0 && (
        <span
          className={`px-2 py-0.5 text-xs font-bold rounded-full ${active ? "bg-white text-cyan-600" : "bg-red-100 text-red-700"}`}
        >
          {count}
        </span>
      )}
              ...
              {todayAppointments.map((apt) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${apt.status === "confirmed" ? "bg-green-100 text-green-700" : apt.status === "pending" ? "bg-yellow-100 text-yellow-700" : "bg-blue-100 text-blue-700"}`}
                    >
                      {apt.status}
                    </span>
                  </div>
                  {apt.status === "confirmed" && (
                    <div className="flex space-x-2">
                      <button
                        ...
                        className="flex-1 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:shadow-lg transition font-semibold text-sm"
                      >
                        Start Call
                      </button>
                      <button
                        onClick={() =>
                          window.confirm("Mark complete?") &&
                          onUpdateStatus(apt._id, "completed")
                        }
                        className="flex-1 py-2 bg-blue-50 text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-100 transition font-semibold text-sm"
                      >
                        Mark Complete
                      </button>
                    </div>
                ...
                >
                  ...
                  <div className="flex space-x-2 mt-3">
                    <button
                      ...
                      className="flex-1 px-3 py-2 bg-green-600 text-white text-sm rounded-xl hover:bg-green-700 transition font-semibold"
                    >
                      ✓ Confirm
                    </button>
                    <button
                      onClick={() => onUpdateStatus(apt._id, "cancelled")}
                      className="flex-1 px-3 py-2 bg-red-600 text-white text-sm rounded-xl hover:bg-red-700 transition font-semibold"
                    >
                      ✕ Decline
                    </button>
                  </div>
          ...
          {["all", "pending", "confirmed", "completed", "cancelled"].map(
            (s) => (
              <button
                key={s}
                onClick={() => setFilter(s)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${filter === s ? "bg-gradient-to-r from-cyan-600 to-blue-600 text-white shadow-md" : "bg-white text-gray-700 border border-gray-300 hover:bg-gray-50"}`}
              >
            ...
            {filtered.map((apt) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${apt.status === "confirmed" ? "bg-green-100 text-green-800" : apt.status === "completed" ? "bg-blue-100 text-blue-800" : apt.status === "cancelled" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}`}
                  >
                    {apt.status}
                  </span>
                </div>
...
function PatientList({ appointments, doctorPatients }) {
  // Merge appointment-derived patient data with the richer doctorPatients list
  const patientsMap = {};
  ...
  // Enrich with email from doctorPatients
  if (doctorPatients) {
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        {patientList.length === 0 ? (
          <p className="text-center text-gray-500 py-12">No patients yet</p>
        ) : (
            ...
            {patientList.map((p, i) => (
              ...
              >
                <div className="flex items-center space-x-3 mb-4">
                  <div className="w-14 h-14 bg-gradient-to-br from-cyan-600 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-md">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between p-2 bg-white rounded-lg">
                    ...
                    <span className="font-bold">{p.appointmentCount}</span>
                  </div>
                  <div className="flex justify-between p-2 bg-white rounded-lg">
        ...
        {upcoming.length === 0 ? (
          ...
          <div className="space-y-4">
            {upcoming.map((apt) => (
              <div
                key={apt._id}
                className="flex items-center p-5 bg-blue-50 rounded-2xl border border-blue-100 hover:shadow-md transition-all"
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">
                    {apt.time} — {apt.reason || "Regular checkup"}
                  </p>
                </div>
                <span
                  className={`px-4 py-2 rounded-xl text-sm font-semibold ${apt.status === "confirmed" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                >
                  {apt.status}
                </span>
              </div>
...
// ─── Doctor Prescriptions and Health Records now use full components ────────
// Removed old view-only functions - using Prescriptions.jsx and HealthRecords.jsx instead
